...
2. **Execute:** run `make emulate SCENARIO=scattered-sable` (15-step chain).
...
| Chain technique coverage | 0% | **100% (12/12)** | `metrics/metrics.json` |
...
| Detection before business impact | none | **~6 min window** | `metrics/metrics.json` |
...
- Added all 15 rules (none existed before).
...
## Coverage gaps found (honest) → backlog
From [`../detections/coverage/coverage-matrix.md`](../detections/coverage/coverage-matrix.md):
- **T1566** phishing (initial access) — no in-lab telemetry; rely on identity + endpoint detections downstream.
- **T1110** credential stuffing/brute force — add a velocity rule on `login_failure`.
- **T1114.003** email forwarding rule — needs a Workspace/Graph audit-log source.
- **T1195** supply-chain — handled by CI gates (Phase 6), not runtime detection.
